...
a = [[0.663,0.666,0.311],[0.363,0.866,0.311],[-0.063,0.866,0.311],[-0.363,0.866,0.311],[-0.863,0.266,0.311],[-0.863,-0.266,0.311],[0.863,-0.266,0.311],[0.863,0.266,0.311],[0.663,0.666,0.0],[0.663,0.666,-0.311]]
b = [82,83,87]

for pos in b:
	for oh in a:
		filename = 're_' +str(pos) + '_oh_' + str(oh[0]) + str(oh[1]) + str(oh[2])
		with open('py/' + filename + '.py', 'w') as f:
			...
			"import numpy as np\n"\
		        "\n"\
			"atoms = fcc111('Pt', size=(5,5,5), vacuum=7.5, a=3.993)\n"\
			"atoms[{pos}].symbol = 'Re'\n"\
