    ...
    for name in names:
       if name.endswith('slab.txt'):
          if name.split("_")[0].isdigit():
            ...
            dirs1 = '/home/theoretical_catalysis/kabelkim/Desktop/masterproject/HCP_vs_FCC/vec_eff_res/HCP/txt'
            dirs2 = '/home/theoretical_catalysis/kabelkim/Desktop/masterproject/HCP_vs_FCC/atom_pos_re_hpc/txt'
            ...
            plt.ylim(-0.03,0.03)
            if int(len(e2)) == 4:
                plt.xticks(x, [str(position.split("_")[0]),str(position.split("_")[1]),"sum",str(position)])
            if int(len(e2)) == 5:
                plt.xticks(x, [str(position.split("_")[0]),str(position.split("_")[1]),str(position.split("_")[2]),"sum",str(position)])
            plt.tight_layout()
            plt.savefig("results/hcp_"+str(position)+".png")
